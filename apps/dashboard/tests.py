...
class DashboardCategoryStatsTest(TestCase):
    ...
    def test_category_percentage_calculation(self):
        ...
        # 퍼센트 합계가 100%인지 확인
        total_percentage = sum(stat['percentage'] for stat in category_stats)
        self.assertAlmostEqual(total_percentage, 100.0, places=1)
        
        # 식비가 60% 정도인지 확인
        food_stat = next((s for s in category_stats if s['category__name'] == '식비'), None)
        self.assertIsNotNone(food_stat)
        self.assertAlmostEqual(food_stat['percentage'], 60.0, places=1)
...
class DashboardUserIsolationTest(TestCase):
    ...
    def test_user_only_sees_own_expenses(self):
        ...
        food_stat = next((s for s in category_stats if s['category__name'] == '식비'), None)
        self.assertIsNotNone(food_stat)
        self.assertEqual(food_stat['total'], Decimal('100000'))  # user2의 20만원은 포함 안 됨